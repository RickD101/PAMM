    ...
    useEffect(() => {
        ...
        findOneCRUD({model: "Component", id: id}).then((response)=>{
            if (response.status){
                ...
                alert('Component does not exist, redirecting to home...');
        ...
        {dataPresent ? 
            <div className={classes.root}>
                ...
                <MaterialTableComponent 
                    model={'Routine'}
                    title={`${componentData.name} Routines`}
                    columns={columns}
                    data={data}
                    setData={setData}
                    pageSize={12}
                    additionalFields = {{owner: componentData._id, ownerModel: 'Component', procedure: []}}
                />
            </div>
            : <Redirect to='/'/>
        }
